...
import useSummernote from "@/app/(admin)/admin/_hooks/useSummernote";
...
export default function ProductEdit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const deletedIds = productImages
        .filter((img) => img.toDelete)
        .map((img) => img.id);
      if (deletedIds.length > 0)
        formData.append("deleted_images", JSON.stringify(deletedIds));

      const res = await adminAxios.post(`/product/${id}`, formData, {
        params: { _method: "PUT" },
      });
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {thumbnail && !newThumbnail && (
            <img src={thumbnail} className="w-32 mb-2 border rounded" />
          )}
          {newThumbnail && (
            <img
              src={URL.createObjectURL(newThumbnail)}
              className="w-32 mb-2 border rounded"
            />
          )}
          <input
