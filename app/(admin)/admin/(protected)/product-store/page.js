...
export default function ProductStoreList() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await adminAxios.delete(`/productstore/${id}`);

      notify.success("Xóa nhập kho thành công!");
      setStores(stores.filter((item) => item.id !== id));
    } catch (err) {
      console.error(err);
      notify.error(
        err.response?.data?.error || "Đã xảy ra lỗi khi xóa bản ghi nhập kho!",
      );
    }
  };
  ...
  return (
    <div className="p-6 text-black min-h-screen flex flex-col">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <a
          href="product-store/add"
          className="bg-blue-500 cursor-pointer px-2 py-1 text-white"
        >
          Nhập kho
        </a>
      </div>
            ...
            stores.map((s) => (
              <tr key={s.id} className="text-center">
                ...
                <td className="border p-2 space-x-4">
                  <a
                    href={`/admin/product-store/${s.id}/edit`}
