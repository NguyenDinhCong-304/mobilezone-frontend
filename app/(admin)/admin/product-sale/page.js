...
export default function ProductSaleList() {
  ...
  const handleImport = async () => {
    if (!file) return alert("Vui lòng chọn file Excel để import!");
    ...
    try {
      ...
      console.error(err);
      alert(err.response?.data?.message || "Lỗi khi import file!");
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        ...
        alert(err.response.data.message || "Yêu cầu không hợp lệ!");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        alert("Không tìm thấy khuyến mãi cần xóa.");
      } else {
